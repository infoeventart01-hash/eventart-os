import { normalizeDateOnly } from "./date-only.mjs";
...
export function validatePaymentDraft(draft) {
  ...
  try {
    if (!normalizeDateOnly(draft.date, "Payment Date")) errors.date = "Select a Payment Date.";
  } catch (error) { errors.date = error.message; }
  try { normalizeDateOnly(draft.due, "Due Date"); }
  catch (error) { errors.due = error.message; }
...
export function buildPaymentFields(draft, context = {}) {
  ...
  const fields = {
    ...
    "Payment Date": normalizeDateOnly(draft.date, "Payment Date"),
  ...
  const dueDate = normalizeDateOnly(draft.due, "Due Date");
