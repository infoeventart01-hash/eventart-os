...
export async function POST(request:NextRequest){
 ...
 try{
  ...
  const results=[];
  for(const file of files){const extension=file.name.split(".").pop()?.toLowerCase()||"";const imagesOnly=attachmentField==="Preview Image"||attachmentField==="Photo";if(!allowedExtensions.has(extension)||(imagesOnly&&!imageExtensions.has(extension)))return NextResponse.json({error:imagesOnly?"Inventory and preview images must be JPG, JPEG, PNG or WEBP":"The selected file type is not supported"},{status:415});if(file.size>maxBytes)return NextResponse.json({error:"This file exceeds the 5 MB limit"},{status:413});
   ...
   const data:unknown=await response.json();if(!response.ok){const error=safeError(data,response.status);console.error("Airtable attachment error",error);return NextResponse.json({error},{status:response.status})}results.push(data)
  }
